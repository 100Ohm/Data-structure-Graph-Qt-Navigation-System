...
bool Graph::getWay(int start, int end){//迪杰特斯拉算法
    if(start > this->vNum || end > this->vNum)
        return false;
    int i;
    int minVex = INF,minRoad = -1;
    int work[MAX_VEX_NUM];//保存当前到该地点权值之和的工作数组
    //初始化
    ...
    for(i = 1; i < this->vNum; i++){//找出离第一个点最近的未知点minVex
        if(work[i] < minRoad && work != 0){
            minRoad = work[i];
            minVex = i;
        }
    }//寻找第一个minVex
